...
class CompressItemJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(HostFilesystemService $fs): void
    {
        ...
        try {
            $fs->compressItem($hosting, $this->relativePath);
            $this->setStatus('done', 'Compress completed.');
        } catch (Throwable $e) {
            $this->setStatus('failed', $e->getMessage());
            Log::warning('file_manager_compress_failed', [
