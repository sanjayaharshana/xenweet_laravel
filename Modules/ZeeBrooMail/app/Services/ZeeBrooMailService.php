...
namespace Modules\ZeeBrooMail\Services;
...
class ZeeBrooMailService
{
    ...
    public function listFolders(Hosting $hosting, int $accountId): array
    {
        ...
        if (! is_array($folders)) {
            return ['ok' => false, 'error' => (string) (imap_last_error() ?: 'Unable to fetch folders.'), 'folders' => []];
        }
        ...
        return ['ok' => true, 'error' => null, 'folders' => $normalized];
    }
    ...
    public function getMessage(Hosting $hosting, int $accountId, string $folder, int $uid): array
    {
        ...
        if ($messageNo <= 0) {
            ...
            return ['ok' => false, 'error' => 'Message not found in selected folder.', 'message' => null];
        }
    ...
    private function openMailboxConnection(Hosting $hosting, int $accountId, string $folder): array
    {
        ...
        if (! $account) {
            return ['ok' => false, 'error' => 'Selected mailbox is invalid for this host.', 'messages' => []];
        }
